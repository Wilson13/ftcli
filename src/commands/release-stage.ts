...
`,
  ...
  static flags = {
    ...
    version: flags.string({char: 'v', description: 'version to build and release'}),
    // flag with a value (-p, --path=VALUE)
    path: flags.string({char: 'p', description: 'path to chart\'s root folder'}),
  }
  ...
  async run() {
    ...
    if (!version) {
      this.log('Argument version (-v/--version) not found, aborting build trigger.')
    }

    if (!path) {
      path = __dirname + '/Chart.yaml';
    }
    ...
    // Get Chart.yaml path
    let chartPath = nodePath.resolve(process.cwd(), path + '/Chart.yaml');
    let chartContent = fs.readFileSync(chartPath, ENCODING);
    let chartYaml = yaml.safeLoad(chartContent);

    // Update app version value with the version provided
    chartYaml.appVersion = version;
    // Serialise object
    let chartYamlStr = yaml.safeDump(chartYaml);
    // Save the file (Chart.yaml)
    fs.writeFileSync(chartPath, chartYamlStr, ENCODING);

    // Get values.yaml path
    let valuesPath = nodePath.resolve(process.cwd(), path + '/values.yaml');
    let valuesContent = fs.readFileSync(valuesPath, ENCODING);
    let valuesYaml = yaml.safeLoad(valuesContent);

    // Update image tag version value with the version provided
    valuesYaml.image.tag = version;
    // Serialise object
    let valuesYamlStr = yaml.safeDump(valuesYaml);
